# SQL注入——一行拼接的代码，拖走了整个数据库

![配图](../../images-new/chapter4/sql-injection.png)
...
SQL注入从1998年被提出到现在，20多年过去了，仍然是OWASP Top 10的常客。不是因为它有多高深，而是因为字符串拼接这个习惯太顽固。

## 核心结论

1. **SQL注入的本质**：用户输入被当作SQL代码执行，攻击者改变了SQL的语义
2. **参数化查询是唯一正确解法**——不是"一种防御手段"，是"唯一正确的写法"
