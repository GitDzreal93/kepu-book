# HTTP协议演进：从HTTP/1.1到HTTP/3的性能瓶颈与解决方案

![](../../images-new/chapter2/http-evolution.png)
...
> HTTP三代协议的进化本质是"把队头阻塞往底层推"——HTTP/1.1阻塞在应用层，HTTP/2阻塞在TCP层，HTTP/3在QUIC层彻底消灭。但每消灭一层，复杂度就增加一层。协议选型别追新，在"性能收益"和"部署成本"之间找平衡点。
