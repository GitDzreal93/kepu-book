# DNS是一个全球分布式目录——一个域名解析请求是怎么在几百毫秒内完成的？

![配图](../../images-new/chapter2/dns-distributed.png)
...
> DNS是互联网的"通讯录"——通过层级划分、分布式缓存、递归迭代，让全球数十亿设备在毫秒级完成域名到IP的翻译。它的设计简洁到近乎简陋，但正是这种简洁让它跑了40年还在用。脆弱？是的。但加了DNSSEC、DoH、Anycast这些补丁后，它依然是互联网最可靠的基础设施之一。
