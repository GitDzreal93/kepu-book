# 分布式ID生成器设计：雪花算法、号段模式与UUID方案对比

![](../../images-new/chapter3/id-generator.png)
...
4. **时钟回拨是Snowflake的阿喀琉斯之踵**。这不是"会不会发生"的问题，而是"发生了怎么处理"。方案从简单（等待时钟追上）到复杂（备用机器ID、Leaf的ZooKeeper方案）各有取舍。
...
一次从数据库领取一个号段（如1000个ID），放在内存里慢慢用，而不是每次生成ID都查数据库。
