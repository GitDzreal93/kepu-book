# 消息队列——Kafka凭什么每秒处理百万条消息

![配图](../../images-new/chapter3/kafka-design.png)
...
4. **ISR（In-Sync Replica）机制**是Kafka在"可靠性"和"性能"之间的平衡术——不是所有副本都同步才返回成功，而是"足够多的副本同步了就返回"，用"可容忍的数据丢失"换取低延迟。
...
> Kafka不是"更快的RabbitMQ"，而是换了一个赛道——用"持久化日志流"重新定义消息队列，把系统复杂度从"Broker管理消费状态"转移到"消费者自己管理进度"，换来了极致的吞吐和回溯能力。理解了这个设计哲学，才能理解它的一切取舍。
