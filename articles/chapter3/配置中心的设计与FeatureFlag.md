# 配置中心设计：动态配置管理与Feature Flag灰度发布

![](../../images-new/chapter3/config-center.png)
...
1. **配置中心的核心价值**在于"变更速度从小时级降到秒级"，而非简单的"不用改代码"。它让运营/产品可以自助变更部分配置，不需要研发参与。
2. **配置推送有三种模式**：定时轮询（简单但有时延）、长连接推送（实时但占连接）、混合模式（长连接+兜底轮询，Apollo采用的方案）。
3. **配置灰度和回滚是配置中心的安全底线**。生效方式是"1%→10%→50%→100%"，而非"所有机器同时生效"，任何阶段异常立即回滚到上一版本。
4. **Feature Flag（特性开关）**是配置中心的进阶玩法。它不只是"开关"，还能"按用户分组、按地区、按比例"控制功能的可见性。它让持续交付成为可能。
