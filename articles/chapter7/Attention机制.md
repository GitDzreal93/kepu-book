## Attention机制到底在"注意"什么？

![配图](../../images-new/chapter7/attention-mechanism.png)

### 从RNN的绝望到Transformer的曙光

2017年我在用LSTM做文本分类——序列长到100个token以上时效果就开始崩。当时我不理解——LSTM有"门"机制，理论上应该能记住长序列信息。

后来读了"Attention Is All You Need"才明白——RNN的瓶颈不是"记不住"，是"信息传递路径太长"。如果第99个词需要参考第3个词的信息——RNN需要一步步传递96步——每一步都有信息衰减。而Attention可以让第99个词直接"看"第3个词——路径长度=1。
...
3. **本质层**：Attention的本质是"动态的、内容感知的加权聚合"——不是靠固定的网络权重，而是靠输入内容本身动态决定"该关注谁"。
...
- 因果注意力（Causal Attention——只看左边的token，不看右边的）对于自回归模型是必须的——否则模型在训练时"偷看答案"。

**一句话**：Attention不是"学会了关注什么"——是"学会了根据输入随时决定关注什么"。
