## 哈希表和B+树——内存索引和磁盘索引的两种设计哲学

![配图](../../images-new/chapter7/hashmap-vs-btree.png)
...
这就是内存数据结构和磁盘数据结构之间的根本性鸿沟：算法的复杂度分析如果不带上"访问模式"和"存储介质特性"——就是纸上谈兵。
...
3. **本质层**：两者不是竞争关系——是"内存vs磁盘"这两种物理约束下各自最优的数据结构。
...
> 哈希表 vs B+树的本质分歧不在算法复杂度上——在**物理世界的代价上**。内存中随机访问和顺序访问差别不大（都是纳秒级），所以哈希表的随机跳转不是问题。磁盘中随机访问和顺序访问差几个数量级（5-10ms vs 顺序吞吐几百MB/s），所以B+树的"把数据聚类在连续的磁盘页上+叶子链表顺序遍历"才是正确的设计。**最好的数据结构永远是被硬件约束最紧的那个——因为硬件约束不能被优化掉。**
