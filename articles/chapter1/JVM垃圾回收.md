# JVM垃圾回收：GC算法对比、分代模型与停顿时间调优

![](../../images-new/chapter1/gc-evolution.png)
...
四代GC算法的演进，是在解决**两个矛盾的优化目标**：回收得快（吞吐量高）和暂停时间短（延迟低）。
...
G1的核心卖点是**可设定的暂停时间目标**：`-XX:MaxGCPauseMillis=200`。G1在目标时间内做尽可能多的回收工作，做不完的留到下次。它不保证200ms以内，只是"尽力而为"。
